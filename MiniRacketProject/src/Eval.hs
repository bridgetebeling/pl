...
evalBoolExpr = do
  ...
  let evals = evalListOfExprs env exprs
  case calcBoolList op evals of
...
evalCompExpr = do
  ...
  case (eval evalExpr (env, a), eval evalExpr (env, b)) of
    (Right (IntValue x, _), Right (IntValue y, _)) -> case op of
        Eq -> return $ BoolValue (x == y)
        Lt -> return $ BoolValue (x < y)
    _ -> typeError "comparison expects two integers"
